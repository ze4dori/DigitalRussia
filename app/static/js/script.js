...
function updateDisplayedContacts() {
    ...
    regionsContainer.innerHTML = ''; // Очищаем текущие контакты

    // Проверяем, есть ли выбранные регионы
    if (selectedRegions.includes("Вся Россия")) {
        // Если выбран "Вся Россия", показываем все контакты
        data.contacts.forEach(contact => {
            addContactToContainer(contact);
        });
    } else if (selectedRegions.length === 0) {
        // Если нет выбранных регионов, показываем все контакты
        data.contacts.forEach(contact => {
            addContactToContainer(contact);
        });
    } else {
        // Показываем только контакты выбранных регионов
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.location.pathname === '/application/new/applicaton-software' || window.location.pathname === '/application/new/applicaton-hardware') {
        ...
        document.getElementById('serviceNames').addEventListener('click', function() {
    ...
    response.forEach(item => {
        ...
        <div class="slideshow-container">
            ...
            <div class="mySlides fade">
                <img src="https://getfile.dokpub.com/yandex/get/${item.main_logo_image}" width="100%">
            </div>
            <div class="mySlides fade">
...
function updateICON(response) {
    ...
    const iconData = [
        { key: 'rutube', src: 'Ico Rutube.png' },
        { key: 'telegram', src: 'Ico Telegram.png' },
        { key: 'dzen', src: 'Ico Dzen.png' },
...
function clickPOOTRASL() {
    ...
    // Initially populate dropdown and show it
    populateDropdown();
    toggleDropdownVisibility(true);
}
...
function submitApplication(type) {
    ...
    const validateRequiredFields = (formData) => {
        ...
        requiredFields.forEach(field => {
            // Для текстовых полей проверка на пустоту
            ...
            // Для checkbox поля проверка на то, что оно выбрано
            if (field.id === 'PolicyError' && !field.value) {
                showError(field.id, field.message); // Отображаем ошибку, если согласие не дано
                isValid = false;
            }
        });
